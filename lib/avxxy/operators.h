...
namespace AVXXY_NAMESPACE
{
	...
	template <typename T, typename S, size_t N> __forceinline SIMD_Mask<S,N> operator==(const SIMD_Vector<S, N>& a, const T& b) { return cmp_equal(a, SIMD_Vector<S, N>(b)); }
	template <typename T, typename S, size_t N> __forceinline SIMD_Mask<S,N> operator!=(const SIMD_Vector<S, N>& a, const T& b) { return cmp_not_equal(a, SIMD_Vector<S, N>(b)); }
	template <typename T, typename S, size_t N> __forceinline SIMD_Mask<S,N> operator>(const SIMD_Vector<S, N>& a, const T& b) { return cmp_greater(a, SIMD_Vector<S, N>(b)); }
	template <typename T, typename S, size_t N> __forceinline SIMD_Mask<S,N> operator<(const SIMD_Vector<S, N>& a, const T& b) { return cmp_less(a, SIMD_Vector<S, N>(b)); }
	template <typename T, typename S, size_t N> __forceinline SIMD_Mask<S,N> operator<=(const SIMD_Vector<S, N>& a, const T& b) { return cmp_less_or_equal(a, SIMD_Vector<S, N>(b)); }
	template <typename T, typename S, size_t N> __forceinline SIMD_Mask<S,N> operator>=(const SIMD_Vector<S, N>& a, const T& b) { return cmp_greater_or_equal(a, SIMD_Vector<S, N>(b)); }
