...
namespace AVXXY_NAMESPACE
{
	...
	__forceinline SIMD_Vector<S, N> unpacklo(const SIMD_Vector<S, N>& a, const SIMD_Vector<S, N>& b)
	{
		...
		else
		{
			...
			for (size_t xmm_i = 0; xmm_i < xmm_count; ++xmm_i) //for each 128-bit lane
			{
				...
				{
					size_t srcI = xmm_i * elements_per_xmm + i / 2;
	...
	static inline float scalar_fp16_to_fp32(uint16_t h)
	{
	...
	__forceinline SIMD_Vector<float, N> vec_cvt_ph2ps(const SIMD_Vector<S, N>& a)
	{
		...
		constexpr auto c = capabilities::current;
		constexpr bool hasVectorVersion = c.FP16C || c.AVX512.F;
		auto x = reinterpret<SIMD_Vector<uint16_t, N>>(a);
		if constexpr (inRange(sizeof(T), 0, 16) && c.FP16C && c.SSE2) return _mm_cvtph_ps(x); //TODO:? FP16C is AVX AFAIK
		else if constexpr (inRange(sizeof(T), 17, 32) && c.FP16C && c.AVX) return _mm256_cvtph_ps(x);
		else if constexpr (inRange(sizeof(T), 33, 64) && c.AVX512.F) return _mm512_cvtph_ps(x);
		else if constexpr (hasVectorVersion) return concat(vec_cvt_ph2ps(x.lo), vec_cvt_ph2ps(x.hi));
		else
		{
			T ret;
			for (size_t i = 0; i < N; ++i) ret[i] = scalar_fp16_to_fp32(x[i]);
			return ret;
		}
	}

	//TODO: verify that it works
	static inline uint16_t scalar_fp32_to_fp16(float x)
	{
		uint32_t f;
		memcpy(&f, &x, sizeof(f));
	...
	__forceinline SIMD_Vector<uint16_t, N> vec_cvt_ps2ph(const SIMD_Vector<float, N>& a)
	{
		...
		constexpr auto c = capabilities::current;
		constexpr bool hasVectorVersion = c.FP16C || c.AVX512.F;
		if constexpr (inRange(sizeof(T), 0, 16) && c.FP16C && c.SSE2) return _mm_cvtps_ph(a, _MM_FROUND_TO_NEAREST_INT); //TODO:? FP16C is AVX AFAIK
		else if constexpr (inRange(sizeof(T), 17, 32) && c.FP16C && c.AVX) return _mm256_cvtps_ph(a, _MM_FROUND_TO_NEAREST_INT);
		else if constexpr (inRange(sizeof(T), 33, 64) && c.AVX512.F) return _mm512_cvtps_ph(a, _MM_FROUND_TO_NEAREST_INT);
		else if constexpr (hasVectorVersion) return concat(vec_cvt_ph2ps(a.lo), vec_cvt_ps2ph(a.hi));
		else
		{
			T ret;
			for (size_t i = 0; i < N; ++i) ret[i] = scalar_fp32_to_fp16(a[i]);
			return ret;
		}
	}
