...
{
public:
	typedef _SSE_Vec4_float Vec4;
	union {
		_SSE_Vec4_float val[4];
		float elements[4][4];
		AVXXY_NAMESPACE::f32x16 zmm;
	};

	Matrix4() {};
	Matrix4(const std::initializer_list<_SSE_Vec4_float> lst);
	...
	__forceinline Vec4_f32x16 operator*(const Vec4_f32x16& v) const
	{
		Vec4_f32x16 ret = 0;
	...
	const _SSE_Vec4_float& operator[](int i) const;
	_SSE_Vec4_float& operator[](int i);
...
public:
	...
	static MatrixPack16_4x4 rotationXYZ(const Vec4_f32x16& angle);
	...
	static MatrixPack16_4x4 fast_rotationXYZ(const Vec4_f32x16& angle);
