...
namespace bob
{
		...
		VectorPack
	{
		...
		PackType len2d() const;
		PackType lenSq() const;
		PackType lenSq3d() const;
		PackType lenSq2d() const;
		...
		PackType dot2d(const VectorPack<PackType>& other) const; //ignore w coordinate
	...
	template <typename PackType>
	inline VectorPack<PackType>::VectorPack(const float f) : x(f), y(f), z(f), w(f) {} //broadcast a single float to all elements

	template <typename PackType>
	inline VectorPack<PackType>::VectorPack(const bob::_SSE_Vec4_float& v) : x(v.x), y(v.y), z(v.z), w(v.w) {};

	template <typename PackType>
	inline VectorPack<PackType>::VectorPack(const PackType& pack) : x(pack), y(pack), z(pack), w(pack) {};
	...
	template <typename PackType>
	inline PackType VectorPack<PackType>::dot2d(const VectorPack<PackType>& other) const
	{
	...
	template<typename PackType>
	inline PackType VectorPack<PackType>::len2d() const
	{
	...
	template <typename PackType>
	inline PackType VectorPack<PackType>::lenSq2d() const
	{
