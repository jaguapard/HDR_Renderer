...
typedef std::function<void()> taskfuncnew_t;
...
private:
	...
	std::unordered_map<uint64_t, ThreadpoolTaskNew> unassignedTasks;
	...
	std::optional<std::pair<uint64_t, ThreadpoolTaskNew>> tryPopTask(); //if there are runnable tasks, then pops one and returns it. Else returns empty optional
