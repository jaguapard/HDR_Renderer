...
Threadpool::Threadpool(size_t numThreads)
{
	...
	if (!numThreads)
	{
		threadCount = std::max(1u, std::thread::hardware_concurrency() - 1);  //hardware_concurrency can return 0. Also, don't crowd out the main thread, unless it is impossible 
	}
	//threadCount = 24;
