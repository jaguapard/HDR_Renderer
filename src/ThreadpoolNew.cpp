#include "ThreadpoolNew.h"
#include <string>
#include <assert.h>
#include "helpers.h"

static thread_local bool bIsWorkerThread = false;
ThreadpoolNew::ThreadpoolNew(size_t numThreads)
{
...
void ThreadpoolNew::spawnThreads(size_t threadCount)
{
	this->workerCount = threadCount;
	this->workers = std::make_unique<std::jthread[]>(workerCount);
	for (size_t i = 0; i < workerCount; ++i)
		this->workers[i] = std::jthread(&ThreadpoolNew::workerRoutine, this, i);
}

std::optional<std::pair<uint64_t, ThreadpoolTaskNew>> ThreadpoolNew::tryPopTask()
{
...
void ThreadpoolNew::markTaskFinished(uint64_t id)
{
	std::lock_guard lck(this->structs_mtx);
...
TaskHandle ThreadpoolNew::addTask(const ThreadpoolTaskNew& task)
{
...
GateHandle ThreadpoolNew::createGate()
{
	GateHandle ret;
...
void ThreadpoolNew::blockUntilTasksComplete(const std::vector<TaskHandle>& tasks)
{
	for (auto& it : tasks)
	{
...
void ThreadpoolNew::blockUntilWaitablesComplete(const std::vector<WaitableHandle>& waitables)
{
	for (auto& it : waitables) this->blockUntilWaitableComplete(it);
}

void ThreadpoolNew::blockUntilWaitableComplete(const WaitableHandle& waitable)
{
...
bool ThreadpoolNew::hasTaskFinished(const TaskHandle& task)
{
	std::lock_guard lck(this->structs_mtx);
...
bool ThreadpoolNew::isGateOpen(const GateHandle& gate)
{
	std::lock_guard lck(this->structs_mtx);
...
void ThreadpoolNew::openGate(const GateHandle& gate)
{
	{
		std::lock_guard lck(this->structs_mtx);
		assert(gate.handle.id < lastFreeHandle);
...
bool ThreadpoolNew::isWorkerThread() const
{
	return bIsWorkerThread;
}

size_t ThreadpoolNew::getWorkerCount() const
{
	return this->workerCount;
}

std::vector<TaskHandle> ThreadpoolNew::addTaskBatch(const std::vector<ThreadpoolTaskNew>& tasks)
{
...
void ThreadpoolNew::workerRoutine(size_t myNumber)
{
	bIsWorkerThread = true;
	while (true)
	{
		std::optional<std::pair<uint64_t, ThreadpoolTaskNew>> taskForMe;
		{
			std::unique_lock cv_lck(cv_mtx);
			cv.wait(cv_lck, [&]() {
...
std::pair<double, double> ThreadpoolNew::getLimitsForThread(size_t threadIndex, double min, double max, std::optional<size_t> threadCount) const
{
