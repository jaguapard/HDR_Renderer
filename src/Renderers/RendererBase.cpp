...
Vec4_f32x16 RendererBase::mask_load_vec4_f32x16_from_framebuffer(const void* frameBuffer, int x, int y, int w, Mask16 mask)
{
	...
	u16x32 rgba0_7 = vcast<u16x32>(load<u64x8>(p + loadInd, mask.lo()));
	u16x32 rgba8_15 = vcast<u16x32>(load<u64x8>(p + loadInd + 8, mask.hi()));
...
void RendererBase::scatterToFrameBuffer(const Vec4_f32x16& colors, int32x16 x, int32x16 y, Mask16 mask, void* frameBuf, int framebufW)
{
	...
	u64x8 rgba0_7 = vcast<u64x8>(permx2(fp16_rg, u16x32(0, 16, 32, 48, 1, 17, 33, 49, 2, 18, 34, 50, 3, 19, 35, 51, 4, 20, 36, 52, 5, 21, 37, 53, 6, 22, 38, 54, 7, 23, 39, 55), fp16_ba));
	u64x8 rgba8_15 = vcast<u64x8>(permx2(fp16_rg, u16x32(8, 24, 40, 56, 9, 25, 41, 57, 10, 26, 42, 58, 11, 27, 43, 59, 12, 28, 44, 60, 13, 29, 45, 61, 14, 30, 46, 62, 15, 31, 47, 63), fp16_ba));
