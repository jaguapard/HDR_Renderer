...
ColorPixelBuffer::ColorPixelBuffer(const SDL_Surface* s)
{
    ...
    if (s->format == SDL_PIXELFORMAT_RGBA32)
    {
        ...
        for (int i = 0; i < totalPixels; i += 32)
        {
            ...
            Vec4_f32x16 p1, p2;
            p1 = Decoder::RGBA8888_to_linear(packed1);
            p2 = Decoder::RGBA8888_to_linear(packed2);
            Mask16 m1 = p1.a >= 1.f;
            Mask16 m2 = p2.a >= 1.f;
...
Vec4_f32x16 ColorPixelBuffer::gatherLinearIntensities(float32x16 u, float32x16 v, Mask16 mask) const
{
    ...
    for (int i = 0; i < 4; ++i)
    {
        ...
        linear[i] = Decoder::RGBA8888_to_linear(samples);
    }
...
Vec4_f32x16 Decoder::RGBA8888_to_linear(int32x16 packed)
{
    Vec4_f32x16 ret;
    ret.a = float32x16(_mm512_cvtepu32_ps(packed >> 24)) * (1.f / 255); //alpha channel is linear already, not gamma encoded
    for (int i = 0; i < 3; ++i)
    {
        int32x16 chan = (packed >> (i * 8)) & 0xFF;
        float32x16 f = _mm512_cvtepu32_ps(chan);
        ret[i] = LUTMan::gamma2p2_toLinear<float32x16, 2>(f * (1.f / 255.f));
    }
    return ret;
}
